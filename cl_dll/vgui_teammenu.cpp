		...
		if (m_pButtons[i])
		{
			if ( i <= gViewPort->GetNumberOfTeams() )
			{
				...
				for ( int j = 1; j < MAX_PLAYERS; j++ )
				{
					...
					if (iTotal > 1)
						Q_strncat( szPlayerList, ", ", sizeof(szPlayerList) - Q_strlen(szPlayerList) );
					Q_strncat( szPlayerList, g_PlayerInfoList[j].name, sizeof(szPlayerList) - Q_strlen(szPlayerList) );
				...
				if (iTotal > 0)
				{
					...
					if (iTotal == 1)
						Q_sprintf(szText, "%s: %d Player (%d points)", gViewPort->GetTeamName(i), iTotal, g_TeamInfo[i].frags );
					else
						Q_sprintf(szText, "%s: %d Players (%d points)", gViewPort->GetTeamName(i), iTotal, g_TeamInfo[i].frags );
					Q_strncat( szText, szPlayerList, sizeof(szText) - Q_strlen(szText) );
